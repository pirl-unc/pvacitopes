...
It:
  ...
  - Outputs a full ranking TSV and a top-N epitopes TSV.
...
def select_best_transcript(group: pd.DataFrame, log_ctx: logger) -> pd.Series:
    ...
    group["Transcript Expression"] = pd.to_numeric(group["Transcript Expression"], errors="coerce")
    group["Protein Position"] = pd.to_numeric(group["Protein Position"], errors="coerce")
    ...
    decision_notes.append(f"Max Expression: {max_expr:.3f}, threshold: {expr_threshold:.3f} (candidates: {len(candidates)})")
    ...
    candidates["TSL_numeric"] = candidates["Transcript Support Level"].apply(get_tsl_value)
    ...
    decision_notes.append(f"Highest Protein Position: {best_protein_pos}, candidates: {len(candidates)}")
    ...
    mutation_id = group.iloc[0][["Chromosome", "Start", "Stop", "Reference", "Variant", "Gene Name"]].to_dict()
...
def adaptive_thresholds(min_val: float, max_val: float, num: int) -> np.ndarray:
    ...
    If num >= 3, the intermediate normalized positions are chosen to approximate
    the default pattern (which for the default range [0.5, 0.95] are [0.75, 0.9] for 4 thresholds;
    more generally, the desired normalized anchors are 0, 0.5556, 0.8889, 1 for n=4;
    for n>4, the intermediate anchors are evenly spaced between 0.5556 and 0.8889).
    ...
    # For n>=3, fix first and last and interpolate intermediate normalized positions.
    # Desired anchors for default (min=0.5, max=0.95) are:
    #   n=4: [0, 0.5556, 0.8889, 1]
    #   n=5: [0, 0.5556, 0.7222, 0.8889, 1] etc.
    # We use linear spacing between 0.5556 and 0.8889 for the n-2 intermediate positions.
    intermediate = np.linspace(0.5556, 0.8889, num - 2)
    thresholds = [min_val] + [min_val + (max_val - min_val) * t for t in intermediate] + [max_val]
...
def main(
    ...
    required_columns = [
        ...
        "Transcript Support Level", "Transcript Expression", "Biotype",
        "Protein Position", "Gene Name", "Mutation",
        "MHCflurryEL Presentation MT Score", "NetMHCpanEL MT Percentile",
        "MHCflurryEL Presentation MT Percentile", "NetMHCpan MT IC50 Score",
        "NetMHCpan MT Percentile", "Tumor DNA VAF"
    ]
    ...
    for name, group in groups:
        try:
            ...
            logger.error("Error selecting best transcript for mutation {}: {}", name, ex)
    if not selected:
    ...
    # For MHCflurry Presentation MT Score thresholds.
    stype = step_type_mhcflurry_pres.lower()
    if stype == "adaptive":
        thr_mhcflurry_pres = adaptive_thresholds(min_threshold_mhcflurry_presentation_score,
                                                  max_threshold_mhcflurry_presentation_score,
                                                  num_threshold_mhcflurry_presentation_score)
    elif stype == "linear":
        thr_mhcflurry_pres = np.linspace(min_threshold_mhcflurry_presentation_score,
                                         max_threshold_mhcflurry_presentation_score,
                                         num_threshold_mhcflurry_presentation_score)
    elif stype == "log2":
        thr_mhcflurry_pres = np.geomspace(max_threshold_mhcflurry_presentation_score,
                                          min_threshold_mhcflurry_presentation_score,
                                          num=num_threshold_mhcflurry_presentation_score)
    else:
        logger.error("Invalid step type for MHCflurry presentation score: {}", step_type_mhcflurry_pres)
        sys.exit(1)

    # For NetMHCpanEL MT Percentile thresholds.
    if step_type_netmhcpanel_el.lower() == "linear":
        thr_netmhcpanel_el = np.linspace(max_threshold_netmhcpanel_el_percentile,
                                         min_threshold_netmhcpanel_el_percentile,
                                         num_threshold_netmhcpanel_el_percentile)
    elif step_type_netmhcpanel_el.lower() == "log2":
        thr_netmhcpanel_el = np.geomspace(max_threshold_netmhcpanel_el_percentile,
                                          min_threshold_netmhcpanel_el_percentile,
                                          num=num_threshold_netmhcpanel_el_percentile)
    else:
        logger.error("Invalid step type for NetMHCpanEL percentile: {}", step_type_netmhcpanel_el)
        sys.exit(1)

    # For MHCflurry Presentation MT Percentile thresholds.
    if step_type_mhcflurry_pres_pct.lower() == "linear":
        thr_mhcflurry_pres_pct = np.linspace(max_threshold_mhcflurry_presentation_percentile,
        ...
        thr_mhcflurry_pres_pct = np.geomspace(max_threshold_mhcflurry_presentation_percentile,
                                              min_threshold_mhcflurry_presentation_percentile,
                                              num=num_threshold_mhcflurry_presentation_percentile)
    else:
        logger.error("Invalid step type for MHCflurry presentation percentile: {}", step_type_mhcflurry_pres_pct)
        sys.exit(1)

    # For NetMHCpan MT IC50 Score thresholds.
    if step_type_netmhcpan_ba.lower() == "linear":
        thr_netmhcpan_ba = np.linspace(max_threshold_netmhcpan_ba,
                                       min_threshold_netmhcpan_ba,
                                       num_threshold_netmhcpan_ba)
    elif step_type_netmhcpan_ba.lower() == "log2":
        thr_netmhcpan_ba = np.geomspace(max_threshold_netmhcpan_ba,
                                        min_threshold_netmhcpan_ba,
    ...
    if step_type_netmhcpan_pct.lower() == "linear":
        thr_netmhcpan_pct = np.linspace(max_threshold_netmhcpan_percentile,
                                        min_threshold_netmhcpan_percentile,
                                        num_threshold_netmhcpan_percentile)
    elif step_type_netmhcpan_pct.lower() == "log2":
        thr_netmhcpan_pct = np.geomspace(max_threshold_netmhcpan_percentile,
                                         min_threshold_netmhcpan_percentile,
                                         num=num_threshold_netmhcpan_percentile)
    else:
        logger.error("Invalid step type for NetMHCpan percentile: {}", step_type_netmhcpan_pct)
        sys.exit(1)
    ...
    logger.info("MHCflurry Presentation Percentile thresholds (<): {}", thr_mhcflurry_pres_pct)
    ...
    for thr in thr_mhcflurry_pres:
        colname = f"thr_mhcflurry_pres_gt_{thr:.3f}"
        df_best[colname] = (df_best["MHCflurryEL Presentation MT Score"] > thr).astype(int)
        threshold_columns.append(colname)
    for thr in thr_netmhcpanel_el:
        colname = f"thr_netmhcpanel_el_lt_{thr:.3f}"
        df_best[colname] = (df_best["NetMHCpanEL MT Percentile"] < thr).astype(int)
        threshold_columns.append(colname)
    for thr in thr_mhcflurry_pres_pct:
        colname = f"thr_mhcflurry_pres_pct_lt_{thr:.3f}"
        df_best[colname] = (df_best["MHCflurryEL Presentation MT Percentile"] < thr).astype(int)
        threshold_columns.append(colname)
    for thr in thr_netmhcpan_ba:
        colname = f"thr_netmhcpan_ba_lt_{thr:.0f}"
        df_best[colname] = (df_best["NetMHCpan MT IC50 Score"] < thr).astype(int)
        threshold_columns.append(colname)
    for thr in thr_netmhcpan_pct:
        colname = f"thr_netmhcpan_pct_lt_{thr:.3f}"
    ...
    df_best["Is_Frameshift"] = df_best["Mutation"].str.contains("fs|frameshift", flags=re.IGNORECASE, na=False)

    # Compute DNA_Score: min(1, Tumor DNA VAF / median(Tumor DNA VAF)).
    median_dna_vaf = df_best["Tumor DNA VAF"].median()
    if median_dna_vaf == 0:
        logger.warning("Median Tumor DNA VAF is 0; setting DNA_Score to 1 for all entries.")
        df_best["DNA_Score"] = 1
    else:
    ...
    df_best["Ranking_Score"] = (df_best["Sum_MHC_Score"] *
                                df_best["DNA_Score"] *
    ...
    try:
        df_best.head(top_n).to_csv(top_output, sep="\t", index=False)
